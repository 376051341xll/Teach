...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="teachers.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
<?php include("top.php");?>
...
<div id="students_main">
	 ...
	 <div id="teachers_main_mid">
	      <div id="teachers_main_mid_top">
			   ...
			   <a href="###" class="teachers_main_mid_memu"  id="dh1" onMouseMove="setDivBG('dh1');souqi('show1')">所有教员</a>
               <a href="###" class="teachers_main_mid_memus" id="dh2" onMouseMove="setDivBG('dh2');souqi('show2')">专业教师</a>
			   <a href="###" class="teachers_main_mid_memus" id="dh3" onMouseMove="setDivBG('dh3');souqi('show3')">专才检索</a>
		  </div>
		  <div id="teachers_main_mid_bot">
		      <table width="100%" id="show1" border="0" cellspacing="0" cellpadding="0">
			   <form name="form1" method="post" action="?action=search_all">
			  <tr>
			   ...
			   <td width="70" align="left"><a href="teachers.php?act=k"><img src="images/xyk_10.jpg"></a></td>
			  </tr>
			  </form>
			</table>
			<table width="100%" id="show2" border="0" cellspacing="0" cellpadding="0" style="display:none;">
			  ...
			  <tr>
			   ...
			   <td width="70" align="left">
		        <input type="image" name="Submit" src="images/xyk_07.jpg"></td>
			   <td width="70" align="left"><a href="teachers.php"><img src="images/xyk_10.jpg"></a></td>
			  </tr>
			  </form>
			</table>
...
<?php
  ...
  $pageSize = PAGESIZE;
  $tbName   = "bw_member";
  $where    = '1=1 and iffb=2 and lang="'.$_COOKIE["cookie_lang"].'" ';
  //搜索
  if(!empty($_GET['action']) && $_GET['action'] == 'search_all')
  {
	 if(!empty($_POST['bh'])){
		$where =$where . " and id LIKE '%".$_POST['bh']."%'"; 
		$_SESSION['whereteacher'] = $where;
	  }
	 if(!empty($_POST['km'])){
  ...
  }elseif(!empty($_GET['action']) && $_GET['action'] == 'search_zyjs')
  {
		if(!empty($_POST['bh'])){
		$where =$where . "and levels=2 and id LIKE '%".$_POST['bh']."%'"; 
		$_SESSION['whereteacher'] = $where;
	  }
	 if(!empty($_POST['km'])){
		$where =$where . "and levels=2  and kjskm LIKE '%".$_POST['km']."%'"; 
		$_SESSION['whereteacher'] = $where;
	  }
	  if(!empty($_POST['byxx'])){
		$where =$where . "and levels=2  and xuexiao LIKE '%".$_POST['byxx']."%'";  
		$_SESSION['whereteacher'] = $where;
	  }
	  if(!empty($_POST['sex'])){
		$where =$where . "and levels=2  and sex = '".$_POST['sex']."'";  
		$_SESSION['whereteacher'] = $where;
	  }
	  if(!empty($_POST['qy'])){
		$where =$where . "and levels=2  and quyu LIKE '%".$_POST['qy']."%'";  
		$_SESSION['whereteacher'] = $where;
	  } 
	  if(!empty($_POST['lx'])){
		$where =$where . "and levels=2";  
		$_SESSION['whereteacher'] = $where;
	  }
  ...
  if($_SESSION['whereteacher']=="")
  {
	$_SESSION['whereteacher'] = $where;  
  }
 //  die($_SESSION['whereteacher']);
  $list = $bw->selectPage("*",$tbName,$_SESSION['whereteacher'],"zjtime desc, id DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['whereteacher'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
         <tr>
		   ...
		   <?php if($list[$i]['levels']==1){echo "大学生";}elseif($list[$i]['levels']==2){echo "职业教师";}elseif($list[$i]['levels']==3){echo "留学,海归";}else{echo "其他";} ?></td>
