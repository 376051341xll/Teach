...
class myConn{
	...
	*/
	...
	function selectPage110($param, $tbname, $where, $order='', $limit='')
	{
		//获得分页数据
	    $pageArray = $this->requestPage($tbname,$where,$limit);
...
//		}
		...
		$sql = "SELECT ".$param." FROM ".$tbname." ".$whereStr." ".$order." ".$limitNum;
		//die($sql);
		//echo $sql;
		$query = $this->query($sql);
		while($rows = $this->returnArray($query))
	...
	*/
	function upload($path,$MAX_FILE_SIZE,$upForm)
	{
		...
		$file_arr=array("image/pjpeg","image/jpeg","image/jpeg","image/gif","image/bmp","image/x-png","image/png","text/plain","application/zip","application/x-zip-compressed","application/octet-stream","application/pdf","application/octet-stream","application/vnd.ms-excel","application/msword","application/vnd.ms-powerpoint","application/x-compress","text/html","audio/mpeg");	//上传文件类型 
