...
if($action == 'delete')
{
	if($bw->delete('bw_moban', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
if($action == 'deleteFrom')
{
	//print_r($_POST);
	$id = implode(',', $_POST['id']);
	if($bw->delete('bw_moban', "id IN (".$id.")"))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<td>关键字:<input name="keyword" id="keyword" />&nbsp;<select name="isshow" id="isshow"><option value="">是否显示</option><option value="1">是</option><option value="0">否</option></select>&nbsp;<input type="submit" value="搜索" /></td>
</form>
</tr>
</table>
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    ...
    <td width="211" align="center"><strong>标题</strong></td>
    <td width="75" align="center"><strong>图片</strong></td>
    <td width="160" align="center"><strong>是否显示</strong></td>
    <td width="53" align="center"><strong>操作</strong></td>
  </tr>
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  if(!empty($_POST['keyword']) && !empty($_POST['isshow']))
	  {
		...
		$_SESSION['wheremoban'] = $where;
	  }else if(!empty($_POST['isshow'])||$_POST['isshow']==0){
  ...
  $list = $bw->selectPage("*",$tbName,$_SESSION['wheremoban'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wheremoban'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><img src="<?php echo $list[$i]['pic'];?>" width="120" alt="" /></td>
    <td align="center"><?php if($list[$i]['isshow'] == 1){echo '是'; }else{echo "<span style='color:red'>否</span>";} ?></td>
    <td align="center"><a href="mobanadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<?php
    if ($list[$i]['id']!=6)
	{
	?><a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a><?php
	}
	?></td>
  </tr>
<?php
