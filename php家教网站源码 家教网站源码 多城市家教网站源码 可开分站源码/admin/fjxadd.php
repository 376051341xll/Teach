...
if(!empty($_GET['id']))
{
	$baseData = $bw->selectOnly('*', 'bw_zxwdhf', 'id = '.$_GET['id'], '');
}
?>
...
<table width="788" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="24">&nbsp;</td>
    <td width="630">&nbsp;
   <input type="hidden" name="id" id="id" value="<?php echo $baseData['id']; ?>" />
   <input name="lang" type="hidden" id="lang" value="<?php if($baseData['lang']==""){echo $Lang;}else{echo $baseData['lang'];}?>" />
   <input type="hidden" name="addtime" id="addtime" value="<?php if(empty($classData['addtime'])){echo date("Y-m-d H:i:s", mktime()); }else{ echo $classData['addtime']; } ?>" />
	</td>
  </tr>
  <tr>
