...
<table width="1030" cellspacing="0" cellpadding="0">
  ...
  <?php
	...
	{
		//print_r($_POST);
		if($bw->insert('bw_exhibitionclass', $_POST))
		{
			$bw->msg('新增成功!', 'zhxxclass.php');	
		}else{
			$bw->msg('新增失败!', 'zhxxclass.php');	
		}
	}
  ...
  //update($tbName, $post, $where)
  	if($action == 'update')
	{
		...
		{
			if($bw->update('bw_exhibitionclass', $_POST, 'id = '.$classId))	
			{
				$bw->msg('更新成功!', 'zhxxclass.php');	
			}else{
				$bw->msg('更新失败!', 'zhxxclass.php');	
			}
		}else{
			$bw->msg('要修改的类别名称不能为空!', 'zhxxclass.php');	
		}
	}
  ?>
  ...
  <?php
  	if($action == 'delete')
	{
		...
		if($id > 4)
		{
			if($bw->delete('bw_exhibitionclass', 'id = '.$_POST['classId']))
			{
				$bw->msg('删除成功!', 'zhxxclass.php');	
			}else{
				$bw->msg('删除失败!', 'zhxxclass.php');	
			}
		}else{
			$bw->msg('系统默认分类不能删除', 'zhxxclass.php');	
		}
	}
  ?>
