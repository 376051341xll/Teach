...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="articlelist.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
</head>
...
<td>关键字:<input name="keyword" id="keyword" />&nbsp;
  ...
  <option value="">选择类别</option>
    <?php
			...
			$list = $bw->selectMany('id, className', 'bw_articleclass', "lang = 'cn' and id not in (4,12)", 'id DESC', '');
			$sum = count($list);
			for($i = 0; $i<$sum; $i++)
			{
		?>
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    ...
    <td width="662" align="center"><strong>标题</strong></td>
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  ...
	  {
		$where = "title LIKE '%".$_POST['keyword']."%' AND classId = '".$_POST['classId']."' and classId not in (4,12)";
		$_SESSION['wherenews'] = $where;
	  }else if(!empty($_POST['classId'])){
		$where = "classId = '".$_POST['classId']."' and classId not in (4,12)";
		$_SESSION['wherenews'] = $where;
	  }else{
		$where = "title LIKE '%".$_POST['keyword']."%' and classId not in (4,12)";  
		$_SESSION['wherenews'] = $where;
	  }
  }else{
       $where = "classId not in (4,12)";
       $_SESSION['wherenews'] = $where;
  }
...
?>
  <tr>
    ...
    <td align="center"><a href="articleadd.php?id=<?php echo $list[$i]['id']; ?>"><?php echo $list[$i]['title']; ?></a></td>
    <td align="center">
    	<?php
			//selectOnly($param,$tbname,$where,$order)
    ...
    <td align="center"><a href="articleadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
