...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="zhxxlist.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
</head>
...
if($action == 'delete')
{
	if($bw->delete('bw_exhibition', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
if($action == 'deleteFrom')
{
	//print_r($_POST);
	$id = implode(',', $_POST['id']);
	if($bw->delete('bw_exhibition', "id IN (".$id.")"))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<?php
  ...
  $list = $bw->selectPage("*",$tbName,$_SESSION['wherezhxx'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherezhxx'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><a href="zhxxadd.php?id=<?php echo $list[$i]['id']; ?>"><?php echo $list[$i]['title']; ?></a></td>
    <td align="center">
    	<?php
			//selectOnly($param,$tbname,$where,$order)
			$className = $bw->selectOnly('className', 'bw_exhibitionclass', 'id = '.$list[$i]['classId'], '');
			echo $className['className'];
		?>
    </td>
    <td align="center"><?php echo $list[$i]['subTime']; ?></td>
    <td align="center"><a href="zhxxadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
