...
<table width="1030" cellspacing="0" cellpadding="0">
  ...
  <?php
	...
	htqx("10.2");
		//print_r($_POST);
		if($bw->insert('bw_articleclass', $_POST))
		{
			$bw->msg('新增成功!', 'articleclass.php');	
		}else{
			$bw->msg('新增失败!', 'articleclass.php');	
		}
	}
  ...
  <tr>
    <td width="263" align="right">添加分类:</td>
    <td width="20">&nbsp;</td>
  ...
  //update($tbName, $post, $where)
	...
	htqx("10.3");
		...
		{
			if($bw->update('bw_articleclass', $_POST, 'id = '.$classId))	
			{
				$bw->msg('更新成功!', 'articleclass.php');	
			}else{
				$bw->msg('更新失败!', 'articleclass.php');	
			}
		}else{
			$bw->msg('要修改的类别名称不能为空!', 'articleclass.php');	
		}
	}
  ?>
  ...
  <?php
	...
	htqx("10.4");
		...
		if($id > 12)
		{
			if($bw->delete('bw_articleclass', 'id = '.$_POST['classId']))
			{
				$bw->msg('删除成功!', 'articleclass.php');	
			}else{
				$bw->msg('删除失败!', 'articleclass.php');	
			}
		}else{
			$bw->msg('系统默认分类不能删除', 'articleclass.php');	
		}
	}
  ?>
