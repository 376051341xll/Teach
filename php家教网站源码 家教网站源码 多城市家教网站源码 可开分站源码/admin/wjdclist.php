...
if($action == 'delete')
{
	if($bw->delete('bw_diaocha', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
if($action == 'deleteFrom')
{
	//print_r($_POST);
	$id = implode(',', $_POST['id']);
	if($bw->delete('bw_diaocha', "id IN (".$id.")"))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    ...
    <td width="153" align="center"><strong>总点击次数</strong></td>
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  if(!empty($_POST['keyword']))
	  {
		...
		$_SESSION['wherezxwd'] = $where;
	  }
  }
  $list = $bw->selectPage("*",$tbName,$_SESSION['wherezxwd'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherezxwd'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><a href="wjdcadd.php?id=<?php echo $list[$i]['id']; ?>"></a><?php echo $list[$i]['title']; ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php
    if($list[$i]['iftj']==1)
	{
	?><span style="color:#F00;">推荐首页</span><?php
	}
	?></td>
    <td align="center">
    	<?php
		$zrs=$list[$i]['Azhi']+$list[$i]['Bzhi']+$list[$i]['Czhi']+$list[$i]['Dzhi']+$list[$i]['Ezhi']+$list[$i]['Fzhi'];
		echo $zrs;		
		?>
    </td>
    <td align="center"><?php echo $list[$i]['addtime']; ?></td>
    <td align="center"><a href="wjdcadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a>&nbsp; <a href="wjdcxiangxi.php?id=<?php echo $list[$i]['id']; ?>">详细信息</a> </td>
  </tr>
<?php
	}//end loop
?>
