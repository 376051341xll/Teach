...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="jyorderlist1.php?page="+thisPage.val();
		});//end page_SEL 		
	});
	function yinc(id)
	{
...
{
htqx("7.3"); 
//	$sql2 = "UPDATE bw_order SET ifdd=2,ddzt=1 where xyid=".$_GET["xyid"]." and jyid=".$_GET["jyid"];
//	$bw->query($sql2);
//	$sql1 = "UPDATE bw_qjj SET zt='正在试教' where id=".$_GET["xyid"];
//	$bw->query($sql1);
//	$bw->msg("安排成功");
...
//$_SESSION['whereorderlist1']="";
  ...
  $pageSize = PAGESIZE;
  $tbName   = "bw_order";
  $where    = "ifdd=1 and yylx=2 and ddzt=1 and lang='".$Lang."' ";
 // die($where);
  //搜索
...
//	    $_SESSION['whereorderlist']=$where;
//  }
  if($_SESSION['whereorderlist1']=="")
  {
	    $_SESSION['whereorderlist1']=$where;
  }
  //die($_SESSION['whereorderlist']);
  $list = $bw->selectPage110("*",$tbName, $_SESSION['whereorderlist1'],"group by xyid  order by id desc",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['whereorderlist1'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
//	  echo $list[$i]['xyid'];
...
?>
    <tr class="showtr">
      ...
      <td align="center"><a href="xyadd.php?id=<?php echo $xynrData['id']?>"><?php echo $xynrData['id']; ?></a></td>
      ...
      <td align="center"><?php if($xynrData['zt']=="正在试教"){echo "<font color='#FF0000'>".$xynrData['zt']."</font>";}else{echo $xynrData['zt'];} ?></td>
  ...
  ?>
	 ...
	 ?></td>
          ...
          <td width="13%" align="center"><a href="?xyid=<?php echo $xynrData['id'];?>&jyid=<?php echo $jynrData['id']; ?>" style="color:#F00;">安排此教员</a></td>
        </tr>
