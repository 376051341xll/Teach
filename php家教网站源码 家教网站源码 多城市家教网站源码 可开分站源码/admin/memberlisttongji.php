<?php
	...
	//unset($_SESSION['wheremembertjlist']);
	$sqlstr="update bw_member set ifrz=3 where daoqi<'".date("Y-m-d")."'";
	$bw->query($sqlstr);
	if($_GET["sou"]=="k")
	{
		$_SESSION['wheremembertjlist']="";
		}
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trusername">
    ...
    <td align="center"><strong>可教授科目</strong></td>
    ...
    <td align="center"><strong>是否通过认证</strong></td>
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  if(!empty($_POST['keyword']) && !empty($_POST['guanjianlei']))
	  {
		...
		$_SESSION['wheremembertjlist'] = $where;
	  }
  ...
  if($_SESSION['wheremembertjlist']=="")
  {
	  $_SESSION['wheremembertjlist'] = $where;
  }
   
// die($_SESSION['wheremembertjlist']);
  $jyqiantj = $bw->selectOnly('sum(Money) as zong,count(Money) as zongr' ,$tbName,$_SESSION['wheremembertjlist']);
  $list = $bw->selectPage("*",$tbName,$_SESSION['wheremembertjlist'],"id DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wheremembertjlist'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
...
?>
 <tr class="showtr">
    ...
    <td align="center"><a href="javascript:if(confirm('确定要修改密码为123456？')){window.location.href='?action=changepwd&id=<?php echo $list[$i]['id']?>'}"><img src="images/pen.gif" alt="重置密码为123456" title="重置密码为123456" /></a>&nbsp;&nbsp;<a onclick="javascript:if(confirm('是否删除')){window.location.href='?action=delete&id=<?php echo $list[$i]['id']; ?>'}" href="###"><img src="images/delete.gif" /></a></td>
  </tr>
...
?>
  ...
  <tr class="trusername">
    <td colspan="14" align="center">  预存款总额：<?php echo $jyqiantj["zong"];?>&nbsp;&nbsp;总人数：<?php echo $jyqiantj["zongr"];?></td>
  </tr>
        	  <tr>
