...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="prolist.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
</head>
...
if($action == 'delete')
{
	if($bw->delete('bw_product', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
if($action == 'deleteFrom')
{
	//print_r($_POST);
	$id = implode(',', $_POST['id']);
	if($bw->delete('bw_product', "id IN (".$id.")"))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    ...
    <td width="662" align="center"><strong>产品名称</strong></td>
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  ...
	  {
		$where = "name LIKE '%".$_POST['keyword']."%' AND classId = ".$_POST['classId'];
		$_SESSION['wherepro'] = $where;
  ...
  $list = $bw->selectPage("*",$tbName,$_SESSION['wherepro'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherepro'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><a href="proadd.php?id=<?php echo $list[$i]['id']; ?>"><?php echo $list[$i]['name']; ?></a></td>
    <td align="center">
    	<?php
			//selectOnly($param,$tbname,$where,$order)
			$className = $bw->selectOnly('className', 'bw_productclass', 'id = '.$list[$i]['classId'], '');
			echo $className['className'];
		?>
    </td>
    <td align="center"><?php echo $list[$i]['subTime']; ?></td>
    <td align="center"><a href="proadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
