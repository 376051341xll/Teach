<?php
	...
	//unset($_SESSION['wherebbs']);
		$updateSql = "UPDATE bw_hygg SET zhuangtai=3 WHERE daoqi<'".date("Y-m-d H:i:s")."'";
		//die($updateSql);
	$bw->query($updateSql);
?>
...
if($action == 'delete')
{
	if($bw->delete('bw_hygg', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    <td width="26" align="center"><strong>ID</strong></td>
    <td width="158" align="center"><strong>标题</strong></td>
    <td width="57" align="center"><strong>申请会员</strong></td>
    <td width="64" align="center"><strong>联系电话</strong></td>
    <td width="62" align="center"><strong>处理状态</strong></td>
    <td width="63" align="center"><strong>申请时间</strong></td>
    <td width="67" align="center"><strong>启用时间</strong></td>
    <td width="60" align="center"><strong>截止日期</strong></td>
    <td width="71" align="center"><strong>操作</strong></td>
  </tr>
...
<?php
  ...
  $list = $bw->selectPage("*",$tbName,$_SESSION['wherehygg'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherehygg'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><?php echo $list[$i]['lianxi']; ?></td>
    <td align="center"><?php
		switch ($list[$i]['zhuangtai'])
		{
			case 1:
			echo "申请中";
			break;
			...
			break;
			default:
			echo "<span style='colro:#ff0000'>已过期</span>";
			break;
			}
		
		?></td>
    <td align="center"><?php echo $list[$i]['addtime']; ?></td>
    <td align="center"><?php echo $list[$i]['kaishi']; ?></td>
    <td align="center"><?php
    if($list[$i]['zhuangtai']==3)
	{
	?><span style='colro:#ff0000'><?php
  }
	?><?php echo $list[$i]['daoqi']; ?><?php
    if($list[$i]['zhuangtai']==3)
	{
	?></span><?php
  }
	?></td>
    <td align="center"><a href="hyggadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
