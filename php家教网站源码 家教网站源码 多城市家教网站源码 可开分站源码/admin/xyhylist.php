...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="xyhylist.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script></head>
...
<td>学员编号:
  <input name="ddbh" id="ddbh" size="8" />
  &nbsp;姓名:
<input name="xyname" id="xyname" size="8" />
  &nbsp;电话:
  <input name="tel" id="tel" size="8" />
  &nbsp;求教科目:
  <input name="qjkm" id="qjkm" size="8" />&nbsp;所在区域:
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trusername">
    ...
    <td width="80" align="center"><strong>学员编号</strong></td>
    <td width="80" align="center"><strong>联系人</strong></td>
    <td width="80" align="center"><strong>学员年级</strong></td>
    <td width="80" align="center"><strong>学员类型</strong></td>
    ...
    <td width="100" align="center"><strong>到期时间</strong></td>
	<td width="100" align="center"><strong>是否到期</strong></td>
...
<?php
  ...
  $pageSize = PAGESIZE;
  $tbName   = "bw_qjj";
  $where    = '1=1 and sfhy =1 and lang="'.$Lang.'" ';
  //搜索
 if(!empty($_GET['action']))
  ...
  if($_SESSION['wherehyxylist']=="")
  {
  $_SESSION['wherehyxylist'] = $where;
  }
  //die($_SESSION['wherehyxylist']);
  $list = $bw->selectPage("*",$tbName, $_SESSION['wherehyxylist'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherehyxylist'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
 <tr class="showtr">
	...
	<td align="center"><?php if($list[$i]['zt']=="正在试教"){echo "<font color='#FF0000'>".$list[$i]['zt']."</font>";}elseif($list[$i]['zt']=="未安排"){echo $list[$i]['zt'];}elseif($list[$i]['zt']=="成功"){echo "<font color='#0000FF'>".$list[$i]['zt']."</font>";} ?></td>
    <td align="center"><input name="xxf<?php echo $list[$i]['id']; ?>" type="text" id="xxf" value="<?php echo $list[$i]['xxf']; ?>" size="10"/></td>
    <td align="center"><?php echo $list[$i]['dqsj']; ?></td>
	<td align="center"><?php if($list[$i]['dqsj']<date("Y-m-d")){echo "<font color='#FF0000'>会员到期</font>";}else{echo "未到期";} ?></td>
	<td align="center"><input name="jyid<?php echo $list[$i]['id']; ?>" type="text" id="jyid" value="<?php
     if($list[$i]['zt']=="正在试教"){
	 	$jyorder = $bw->selectOnly('jyid', 'bw_order', 'xyid = '.$list[$i]['id'].' and ifdd=2', '');
		echo $jyorder["jyid"];
	 }
	?>" size="10"/></td>
...
?>
  <tr>
  	<td colspan="14" align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
    	  <tr>
