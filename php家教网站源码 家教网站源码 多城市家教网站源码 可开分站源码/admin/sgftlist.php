...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="sgftlist.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
</head>
...
if($action == 'delete')
{
	if($bw->delete('bw_h2o', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<?php
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  if(!empty($_POST['keyword']) && !empty($_POST['classId']))
	  {
		...
		$_SESSION['wheresgft'] = $where;
	  }else{
		$where = "title LIKE '%".$_POST['keyword']."%'";  
		$_SESSION['wheresgft'] = $where;
	  }
  }
  $list = $bw->selectPage("*",$tbName,$_SESSION['wheresgft'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wheresgft'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><a href="sgftadd.php?id=<?php echo $list[$i]['id']; ?>"><?php echo $list[$i]['title']; ?></a></td>
    <td align="center">&nbsp;</td>
    <td align="center"><?php echo $list[$i]['subTime']; ?></td>
    <td align="center"><a href="sgftadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
