...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="orderwap.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script></head>
...
htqx("7.3"); 
	...
	$sql = "UPDATE bw_qjj SET zt = '正在试教' WHERE id = ".$_GET['id'];
	//die($sql);
	if($bw->query($sql))
	{
		$bw->msg('处理成功!', 'orderwap.php');	
	}else{
		$bw->msg('处理失败!', 'orderwap.php');	
	}
}
?>
...
<?php
  ...
  $pageSize = PAGESIZE;
  $tbName   = "bw_qjj";
  $where    = "1=1 and zt='未安排' and isshow=2 and lang='".$Lang."' ";
  //搜索
...
?>
 <tr class="showtr">
    ...
    <td align="center"><a href="###" title="<?php echo $list[$i]['qjkm']; ?>"><?php echo $list[$i]['qjkm']; ?></a></td>
    ...
    <td align="center"><!--<a href="?action=ggzt&id=<?php echo $list[$i]['id']?>"><img src="images/pen.gif" alt="更改状态" title="更改状态" />--></a><a href="xyadd.php?id=<?php echo $list[$i]['id']?>"><img src="images/pen.gif" alt="修改信息" title="修改信息" /></a>&nbsp;&nbsp;<a href="###" onclick="javascript:if(confirm('是否删除')){window.location.href='?action=delete&id=<?php echo $list[$i]['id']; ?>'}"><img src="images/delete.gif" /></a></td>
  </tr>
<?php
	}//end loop
?>
  <tr>
  	<td colspan="12" align="center">
    	<div class="pageDiv">
            ...
            <div class="pageRight" style="width:500px;">
