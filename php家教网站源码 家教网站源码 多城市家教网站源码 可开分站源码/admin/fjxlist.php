...
{
htqx("13_4");
	if($bw->delete('bw_zxwdhf', 'id = '.$_GET['id']))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
htqx("13_4");
	//print_r($_POST);
	$id = implode(',', $_POST['id']);
	if($bw->delete('bw_zxwdhf', "id IN (".$id.")"))
	{
		$bw->msg('删除成功!');	
	}else{
		$bw->msg('删除失败!');	
...
<table width="96%" cellpadding="0" cellspacing="0">
  <tr class="trTitle">
    ...
    <td width="250" align="center"><strong>收件人</strong></td>
 ...
 $where    = 'lang="'.$Lang.'" ';
  ...
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	  if(!empty($_POST['keyword']))
	  {
		...
		$_SESSION['wherefjx'] = $where;
	  }
  }
  $_SESSION['wherefjx'] = $where;
   //die($_SESSION['wherefjx']);
  $list = $bw->selectPage("*",$tbName,$_SESSION['wherefjx'],"`id` DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wherefjx'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
  <tr>
    ...
    <td align="center"><a href="#" onclick="yinc('a<?php echo $list[$i]['id'];?>')"><?php 
	//echo $list[$i]['uid'];
	if(!empty($list[$i]['uid']))
	{
		$hynameData = $bw->selectOnly('username', 'bw_member', 'id='.$list[$i]['uid'], '');
			echo $hynameData["username"];
		} ?></a></td>
	 <td align="center"><a href="#" onclick="yinc('a<?php echo $list[$i]['id'];?>')">
	 <?php if($list[$i]['xxlx']==1){echo "单条信息";}else{echo "群发信息";} ?></a></td>
    ...
    <td align="center"><a href="fjxadd.php?id=<?php echo $list[$i]['id']; ?>"><img src="images/pen.gif" /></a>&nbsp;&nbsp;<a href="?action=delete&id=<?php echo $list[$i]['id']; ?>"><img src="images/delete.gif" /></a></td>
  </tr>
  <tr id="a<?php echo $list[$i]['id'];?>" style="display:none;" class="trTitle">
    <td align="center"><input type="checkbox" name="id[]" id="id[]" value="<?php echo $list[$i]['id']; ?>" /></td>
    <td align="center"><strong>内容</strong></td>
    <td colspan="3" align="left"><?php echo $list[$i]['content']; ?></td>
    </tr>
<?php
	}//end loop
?>
  <tr>
  	<td colspan="6" align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
    	  <tr>
    	    <td><input type="button" value="全选" id="selectAll" />&nbsp;<input type="button" value="反选" id="orderSelect" />&nbsp;<input type="submit" value="删除所选" id="deleteSelect" /></td>
