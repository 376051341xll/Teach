...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="mxjy.php?page="+thisPage.val();
		});//end page_SEL 		
	});
</script>
<BODY>
...
<div id="all_main_all">
	 ...
	 <div id="mxjy_box">
	   ...
	   <div id="mxjy_box_mid">
			...
			<a href="mxjy.php?act=k" class="mxjy_box_mid_dh_memu">明星教员</a>
            <a href="mxjys.php?act=k" class="mxjy_box_mid_dh_memus" >明星大学生</a>
			</div>
...
<?php
  ...
  $pageSize = PAGESIZE;
  $tbName   = "bw_member";
  $where    = '1=1 and iffb=2 and ifxj>=3 and ifxj<6 and lang="'.$_COOKIE["cookie_lang"].'" ';
  //搜索
  if(!empty($_GET['action']) && $_GET['action'] == 'search')
  {
	 if(!empty($_POST['bh'])){
		$where =$where . " and id LIKE '%".$_POST['bh']."%'"; 
		$_SESSION['wheremxjy'] = $where;
	  }
	 if(!empty($_POST['km'])){
  ...
  if($_SESSION['wheremxjy']=="")
  {
	$_SESSION['wheremxjy'] = $where;  
  }
   //die($_SESSION['wheremxjy']);
  $list = $bw->selectPage("*",$tbName,$_SESSION['wheremxjy'],"zjtime desc, id DESC",$pageSize);
  $pageArray = $bw->requestPage($tbName,$_SESSION['wheremxjy'],$pageSize);
  $sum = count($list);
  for($i = 0; $i<$sum; $i++)
  {
?>
